...
public sealed class IdeCallStackPopupTests
{
    ...
    public async Task RendersCallStackNodesFromState()
    {
        ...
        var callStackState = new IdeCallStackState(new Dictionary<string, IdeCallStackViewState>(StringComparer.OrdinalIgnoreCase)
        {
            [solutionId] = new IdeCallStackViewState(
                true,
                false,
                false,
                null,
