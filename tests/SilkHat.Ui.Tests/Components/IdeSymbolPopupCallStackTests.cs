...
namespace SilkHat.Ui.Tests.Components
{
    public sealed class IdeSymbolPopupCallStackTests
    {
        ...
        public async Task CallStackButtonDispatchesOpenAction_ForSelectedMethod()
        {
            ...
            {
                new("symbol-key", "M:Samples.Program.Run", "Run", "Method", "Method", null,
                    Array.Empty<CodeSymbolOutlineNodeModel>())
            };
            var symbolsState = new IdeSymbolsState(
                new Dictionary<string, IdeSymbolsViewState>(StringComparer.OrdinalIgnoreCase)
                {
                    [solutionId] = new(true, false, null, "./Program.cs", nodes, "M:Samples.Program.Run", "symbol-key")
                });
            ...
            provider.WaitForAssertion(() => { Assert.Contains("Call Stack", provider.Markup); });
