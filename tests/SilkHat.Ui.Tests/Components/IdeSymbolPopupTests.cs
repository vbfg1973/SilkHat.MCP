        ...
        public async Task RendersSymbolCardsFromState()
        {
            ...
            var nodes = new List<CodeSymbolOutlineNodeModel>
            {
                new(
                    ...
                    {
                        new("sym-2", "M:Samples.ComplexitySamples.CalculateScore(System.Int32)", "CalculateScore",
                            "Method", "Method", null, Array.Empty<CodeSymbolOutlineNodeModel>())
                    })
            };

            var symbols = new IdeSymbolsState(
                new Dictionary<string, IdeSymbolsViewState>(StringComparer.OrdinalIgnoreCase)
                {
                    [solutionId] = new(true, false, null, "./AnalysisSamples.cs", nodes, null, null)
                });
