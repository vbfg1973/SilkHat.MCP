    ...
    public sealed class MethodImplementationDecisionServiceTests
    {
        ...
        public async Task ResolveAsync_UsesImplementationDocId_WhenStoredDecisionProvidesIt()
        {
            ...
            var implementationMethod =
                FindMethodSymbol(solution, "SilkHat.Sample.App.FriendlyGreetingProvider", "GetGreeting");
            var signature = BuildInterfaceMethodSignature(interfaceMethod);
        ...
        private static async Task<(CodeRepositoryWorkspace Workspace, CodeSolutionWorkspace Solution, IMethodSymbol
                InterfaceMethod)>
            BuildCustomWorkspaceAsync()
        {
            ...
            var emitResult = await appCompilation.EmitAsync(metadataStream, CancellationToken.None);
            if (!emitResult.Success)
            {
                var diagnostics = string.Join(Environment.NewLine,
                    emitResult.Diagnostics.Select(item => item.ToString()));
            ...
            var solution = new CodeSolutionWorkspace(
                ...
                {
                    ["app"] = new("app", "App", "C#", "App", Array.Empty<CodeProjectReferenceDto>(),
                        Array.Empty<CodeProjectReferenceDto>()),
                    ["app.tests"] = new("app.tests", "App.Tests", "C#", "App.Tests",
                        Array.Empty<CodeProjectReferenceDto>(), Array.Empty<CodeProjectReferenceDto>())
                },
