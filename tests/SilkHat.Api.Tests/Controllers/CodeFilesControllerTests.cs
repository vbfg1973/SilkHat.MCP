...
namespace SilkHat.Api.Tests.Controllers
{
    public sealed class CodeFilesControllerTests
    {
        ...
        public async Task GetFile_ReturnsProblem_WhenWorkspaceMissing()
        {
            ...
            var result = await controller.GetFile(Guid.NewGuid(), CodeWorkspaceFactory.DefaultSolutionId,
                new CodeFileQuery { Path = "Repo/Program.cs" }, CancellationToken.None);
        ...
        public async Task GetFile_ReturnsOk_WhenFileFound()
        {
            ...
            fileService
                .Setup(s => s.GetFileAsync(workspace, solution, "Repo/Program.cs", It.IsAny<CancellationToken>()))
            ...
            fileService.Verify(
                s => s.GetFileAsync(workspace, solution, "Repo/Program.cs", It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        public async Task GetFile_ReturnsValidationProblem_WhenModelInvalid()
        {
            ...
            var result = await controller.GetFile(Guid.NewGuid(), CodeWorkspaceFactory.DefaultSolutionId,
                new CodeFileQuery(), CancellationToken.None);
