...
public sealed class GitFilesControllerTests
{
    ...
    public async Task GetHistory_ReturnsProblem_WhenRepositoryNotLoaded()
    {
        ...
        var result = await controller.GetHistory(Guid.NewGuid(), "src/Program.cs", new PagingQuery(), CancellationToken.None);
    ...
    public async Task GetHistory_ReturnsEntries()
    {
        ...
        var result = await controller.GetHistory(configId, "src/Program.cs", new PagingQuery(), CancellationToken.None);
    ...
    public async Task GetCoChanges_ReturnsEntries()
    {
        ...
        var result = await controller.GetCoChanges(configId, "src/Program.cs", new PagingQuery(), CancellationToken.None);
    ...
    public async Task GetLastChange_ReturnsCommitMetadata()
    {
        ...
        store.Setup(s => s.Get(configId)).Returns(new LoadedRepository(configId, "/repo", DateTimeOffset.UtcNow));
        var gitCli = new Mock<IGitCli>();
        gitCli.Setup(c => c.GetFileLastChangeAsync(configId, "/repo", "src/Program.cs", true, It.IsAny<CancellationToken>()))
