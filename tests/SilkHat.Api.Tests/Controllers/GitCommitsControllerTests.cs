...
public sealed class GitCommitsControllerTests
{
    ...
    public async Task GetCommits_ReturnsProblem_WhenRepositoryMissing()
    {
        ...
        store.Setup(s => s.Get(It.IsAny<Guid>())).Returns((SilkHat.Analysis.Models.LoadedRepository?)null);
    ...
    public async Task GetCommits_ReturnsEntries()
    {
        var store = new Mock<ILoadedRepositoryStore>();
        var gitCli = new Mock<IGitCli>();
        var repo = new SilkHat.Analysis.Models.LoadedRepository(Guid.NewGuid(), "/repo", DateTimeOffset.UtcNow);
        store.Setup(s => s.Get(It.IsAny<Guid>())).Returns(repo);
        var commits = new List<GitCommitDto>
        {
            new("sha", "sha", Array.Empty<string>(), "Author", "author@example.com", DateTimeOffset.UtcNow, "Subject", "Body", false, new List<GitCommitFileChangeDto>())
        };
