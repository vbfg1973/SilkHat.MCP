...
""");
        ...
        try
        {
            ...
            Assert.Contains(parsed.CompileItems, path => path.EndsWith("Explicit.cs", StringComparison.OrdinalIgnoreCase));
            Assert.Contains(parsed.CompileItems, path => path.EndsWith("Other.cs", StringComparison.OrdinalIgnoreCase));
            Assert.DoesNotContain(parsed.CompileItems, path => path.EndsWith("Ignored.cs", StringComparison.OrdinalIgnoreCase));
        }
        finally
        {
