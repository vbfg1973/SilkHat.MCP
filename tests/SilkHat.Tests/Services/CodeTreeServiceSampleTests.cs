...
namespace SilkHat.Tests.Services
{
    public sealed class CodeTreeServiceSampleTests
    {
        ...
        public async Task GetTree_ReturnsTypeMembers_FromSampleSolution()
        {
            ...
            var members =
                await service.GetTreeAsync(workspace, solution, complexityType.DisplayPath, CancellationToken.None);
            ...
            var helperMembers =
                await service.GetTreeAsync(workspace, solution, helperType!.DisplayPath, CancellationToken.None);
            Assert.Contains(helperMembers,
                member => member.Name == "Grade11PlusScore" && member.RealType == "Property");
