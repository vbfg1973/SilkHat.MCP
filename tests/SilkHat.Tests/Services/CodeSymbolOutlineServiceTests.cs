...
public sealed class CodeSymbolOutlineServiceTests
{
    ...
    public async Task GetFileSymbols_ReturnsNamedTypesAndMembers()
    {
        ...
        var result = await service.GetFileSymbolsAsync(workspace, solution, entry.RepositoryPath, CancellationToken.None);
        ...
        Assert.Contains(complexity!.Children, child => child.Name == "CalculateScore" && child.RealType == "Method");

        var helper = result.Symbols!.FirstOrDefault(node => node.Name == "StatusHelper");
        Assert.NotNull(helper);
        Assert.Contains(helper!.Children, child => child.Name == "Grade11PlusScore" && child.RealType == "Property");
    }
