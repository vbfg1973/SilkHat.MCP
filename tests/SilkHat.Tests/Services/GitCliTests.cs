...
namespace SilkHat.Tests.Services
{
    public sealed class GitCliTests
    {
        ...
        public async Task ListTreeAsync_IncludesDirectoriesAndFiles()
        {
            ...
            var entries =
                await cli.ListTreeAsync(Guid.NewGuid(), "/repo", null, null, null, null, CancellationToken.None);
        ...
        public async Task FileHistoryAsync_ParsesCommits()
        {
            ...
            var history = await cli.FileHistoryAsync(Guid.NewGuid(), "/repo", "src/Program.cs", 1, 50,
        ...
        public async Task CoChangeStatsAsync_CountsOtherFiles()
        {
            ...
            var stats = await cli.CoChangeStatsAsync(Guid.NewGuid(), "/repo", "src/Program.cs", 1, 50,
                CancellationToken.None);
        ...
        public async Task GetFileLastChangeAsync_ParsesDiffLines()
        {
            var runner = new FakeGitCommandRunner();
            runner.Add(
                "log -n 1 --date=iso-strict --pretty=format:%H%x09%h%x09%an%x09%ae%x09%ad%x09%s --follow -- src/Program.cs",
            ...
            var change = await cli.GetFileLastChangeAsync(Guid.NewGuid(), "/repo", "src/Program.cs", true,
                CancellationToken.None);
        ...
        public async Task GetFileChangeCountAsync_ReturnsCommitCount()
        {
            ...
            var count = await cli.GetFileChangeCountAsync(Guid.NewGuid(), "/repo", "src/Program.cs",
                CancellationToken.None);
        ...
        public async Task GetFileAuthorCountAsync_ReturnsDistinctAuthorCount()
        {
            ...
            var count = await cli.GetFileAuthorCountAsync(Guid.NewGuid(), "/repo", "src/Program.cs",
                CancellationToken.None);
