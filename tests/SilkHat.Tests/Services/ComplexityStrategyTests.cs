...
public sealed class ComplexityStrategyTests
{
    ...
    private static async Task<(BaseMethodDeclarationSyntax MethodSyntax, SemanticModel SemanticModel)> LoadMethodAsync()
    {
        ...
        var compilation = solution.Compilations.Values.First(compilation =>
            string.Equals(compilation.AssemblyName, method.ContainingAssembly?.Name, StringComparison.OrdinalIgnoreCase));
