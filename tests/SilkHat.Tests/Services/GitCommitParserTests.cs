...
public sealed class GitCommitParserTests
{
    ...
    public void Parse_ExtractsCommitMetadataAndChanges()
    {
        ...
        Assert.Contains(first.Changes, change => change.ChangeKind == GitChangeKind.Modify && change.Path == "./docs/README.md");
        Assert.Contains(first.Changes, change => change.ChangeKind == GitChangeKind.Add && change.Path == "./docs/api.md");
        Assert.Contains(first.Changes, change => change.ChangeKind == GitChangeKind.Rename && change.OldPath == "./docs/old.md" && change.Path == "./docs/new.md");
