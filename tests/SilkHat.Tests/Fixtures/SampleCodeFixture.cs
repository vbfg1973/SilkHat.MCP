...
namespace SilkHat.Tests.Fixtures
{
    ...
    public sealed class SampleCodeFixture
    {
        ...
        public Task<FileComplexityMetrics> GetCachedMetricsAsync()
        {
            return _metrics.Value;
        }

        private static async
            Task<(CodeRepositoryWorkspace Repo, CodeSolutionWorkspace Solution, IComplexityMetricsAggregator Aggregator
                )> BuildWorkspaceAsync()
        {
            var rootPath = "/repo";
            ...
            var repositoryWorkspace = new CodeRepositoryWorkspace(rootPath,
