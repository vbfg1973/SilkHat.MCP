...
namespace SilkHat.Ui.State.Ide.Effects
{
    public sealed class IdeCallStackEffects
    {
        ...
        public Task HandleOpenPopup(OpenCallStackPopupAction action, IDispatcher dispatcher)
        {
            ...
            if (configId is null)
            {
                dispatcher.Dispatch(new LoadCallStackFailureAction(action.SolutionId, action.DocumentationId,
                    action.SymbolKey, "Solution not found."));
        ...
        {
            _logger.LogDebug("IDE: loading call stack for docId {DocumentationId} symbolKey {SymbolKey}",
                action.DocumentationId, action.SymbolKey);
            try
            {
                ...
                dispatcher.Dispatch(new LoadCallStackSuccessAction(action.SolutionId, action.DocumentationId,
                    action.SymbolKey, response.Nodes));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "IDE: failed to load call stack for docId {DocumentationId} symbolKey {SymbolKey}",
                    action.DocumentationId, action.SymbolKey);
                dispatcher.Dispatch(new LoadCallStackFailureAction(action.SolutionId, action.DocumentationId,
                    action.SymbolKey, ex.Message));
            }
        }
        ...
        {
            _logger.LogDebug("IDE: loading call stack mermaid for docId {DocumentationId} symbolKey {SymbolKey}",
                action.DocumentationId, action.SymbolKey);
            try
            {
                var response = await _api.GetMethodCallStackMermaidAsync(
                    ...
                    new MethodCallStackRequestModel(action.DocumentationId, action.SymbolKey, action.MaxDepth,
                        action.IncludeExternalCalls));
                dispatcher.Dispatch(new LoadCallStackMermaidSuccessAction(action.SolutionId, action.DocumentationId,
                    action.SymbolKey, response.Diagram));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    "IDE: failed to load call stack mermaid for docId {DocumentationId} symbolKey {SymbolKey}",
                    action.DocumentationId, action.SymbolKey);
                dispatcher.Dispatch(new LoadCallStackMermaidFailureAction(action.SolutionId, action.DocumentationId,
                    action.SymbolKey, ex.Message));
            }
        }
        ...
        public async Task HandleSaveDecision(SaveCallStackDecisionAction action, IDispatcher dispatcher)
        {
            ...
            try
            {
                ...
                dispatcher.Dispatch(
                    new SaveCallStackDecisionSuccessAction(action.SolutionId, action.InterfaceMethodSignature));
            ...
            {
                _logger.LogError(ex, "IDE: failed to save call stack decision for {Signature}",
                    action.InterfaceMethodSignature);
                dispatcher.Dispatch(new SaveCallStackDecisionFailureAction(action.SolutionId,
                    action.InterfaceMethodSignature, ex.Message));
            }
        }
