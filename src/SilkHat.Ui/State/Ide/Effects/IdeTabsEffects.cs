...
namespace SilkHat.Ui.State.Ide.Effects
{
    public sealed class IdeTabsEffects
    {
        ...
        {
            if (action.Entry.Type != CodeTreeEntryType.File) return;
            ...
            try
            {
                ...
                {
                    var lastChange =
                        await _api.GetGitFileLastChangeAsync(action.ConfigId, action.Entry.RepositoryPath, false);
                    ...
                    var changeCount =
                        await _api.GetGitFileChangeCountAsync(action.ConfigId, action.Entry.RepositoryPath);
                    tab.ChangeCount = changeCount.ChangeCount;
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "IDE: unable to load last commit metadata for {Path}",
                        action.Entry.DisplayPath);
                }
        ...
        public async Task HandleOpenFileByPath(OpenFileTabByPathAction action, IDispatcher dispatcher)
        {
            ...
            try
            {
                ...
                var displayPath = string.IsNullOrWhiteSpace(content.DisplayPath)
        ...
        public async Task HandleToggleDiff(ToggleDiffAction action, IDispatcher dispatcher)
        {
            ...
            var view = _tabsState.Value.Tabs.TryGetValue(action.SolutionId, out var state)
                ? state
                : null;
            if (view is null || view.OpenFiles.Count == 0) return;

            var index = view.ActiveTabIndex;
            if (index < 0 || index >= view.OpenFiles.Count) return;
            ...
            var solutionEntry = _solutionsState.Value.Solutions
                .FirstOrDefault(solution =>
                    string.Equals(solution.SolutionId, action.SolutionId, StringComparison.OrdinalIgnoreCase));
            if (solutionEntry is null) return;

            _logger.LogDebug("IDE: loading diff for {Path}", file.DisplayPath);
            try
            {
                var lastChange =
                    await _api.GetGitFileLastChangeAsync(solutionEntry.ConfigId, file.RepositoryPath, true);
                ...
                {
                    var changeCount =
                        await _api.GetGitFileChangeCountAsync(solutionEntry.ConfigId, file.RepositoryPath);
