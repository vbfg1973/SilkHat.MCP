    ...
    public sealed class IdeTreeEffects
    {
        ...
        public async Task HandleLoadTreeRoot(LoadTreeRootAction action, IDispatcher dispatcher)
        {
            ...
            try
            {
                ...
                var tree = await _api.GetCodeTreeAsync(
                    ...
                    action.SolutionId,
                    null,
                    settings.IsEnabled ? settings.AnnotationKind : null,
                    settings.IsEnabled ? settings.FilterMetric : null,
                    settings.IsEnabled ? settings.FilterOperator : null,
                    settings.IsEnabled ? settings.FilterThreshold : null);
        ...
        private static TreeItemData<CodeTreeEntryModel> BuildTreeItem(CodeTreeEntryModel entry)
        {
            return new TreeItemData<CodeTreeEntryModel>
            {
                ...
                Expandable = entry.Type is CodeTreeEntryType.Project or CodeTreeEntryType.Directory
                    or CodeTreeEntryType.File or CodeTreeEntryType.Type,
                Expanded = false
            };
        }
        ...
        private Task ReloadTree(IDispatcher dispatcher)
        {
            ...
            if (string.IsNullOrWhiteSpace(selected)) return Task.CompletedTask;

            var entry = _solutionsState.Value.Solutions
                .FirstOrDefault(solution =>
                    string.Equals(solution.SolutionId, selected, StringComparison.OrdinalIgnoreCase));
            if (entry is null) return Task.CompletedTask;
