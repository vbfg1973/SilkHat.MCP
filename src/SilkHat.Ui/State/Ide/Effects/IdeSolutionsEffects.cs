    ...
    public sealed class IdeSolutionsEffects
    {
        ...
        public async Task HandleLoadSolutions(LoadSolutionsAction action, IDispatcher dispatcher)
        {
            ...
            try
            {
                ...
                {
                    var nameComparison = string.Compare(left.DisplayName, right.DisplayName,
                ...
                if (solutions.Count > 0) dispatcher.Dispatch(new SelectSolutionAction(solutions[0].SolutionId));
            }
            catch (Exception ex)
            {
