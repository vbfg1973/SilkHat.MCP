...
public sealed class IdeSymbolsEffects
{
    ...
    public Task HandleOpenPopup(OpenSymbolPopupAction action, IDispatcher dispatcher)
    {
        ...
        {
            dispatcher.Dispatch(new LoadFileSymbolsFailureAction(action.SolutionId, string.Empty, "No active file tab."));
        ...
        if (configId is null)
        {
            dispatcher.Dispatch(new LoadFileSymbolsFailureAction(action.SolutionId, activeFile.RepositoryPath, "Solution not found."));
            return Task.CompletedTask;
        }

        dispatcher.Dispatch(new LoadFileSymbolsAction(action.SolutionId, configId.Value, activeFile.RepositoryPath));
        return Task.CompletedTask;
    }
    ...
    public Task HandleOpenFileTabSuccess(OpenFileTabSuccessAction action, IDispatcher dispatcher)
    {
        ...
        dispatcher.Dispatch(new LoadFileSymbolsAction(action.SolutionId, configId.Value, action.Tab.RepositoryPath));
        return Task.CompletedTask;
    }
    ...
    public async Task HandleLoadSymbols(LoadFileSymbolsAction action, IDispatcher dispatcher)
    {
        ...
        try
        {
            var nodes = await _api.GetCodeFileSymbolsAsync(action.ConfigId, action.SolutionId, action.RepositoryPath);
        ...
        catch (Exception ex)
        {
            ...
            dispatcher.Dispatch(new LoadFileSymbolsFailureAction(action.SolutionId, action.RepositoryPath, ex.Message));
        }
    }
    ...
    {
        if (!_tabsState.Value.Tabs.TryGetValue(solutionId, out var view))
        {
            return null;
        }

        if (view.OpenFiles.Count == 0)
        {
            return null;
        }
