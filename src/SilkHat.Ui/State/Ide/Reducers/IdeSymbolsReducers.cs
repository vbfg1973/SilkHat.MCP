...
public static class IdeSymbolsReducers
{
    ...
    [ReducerMethod]
    public static IdeSymbolsState ReduceLoadSymbolsSuccess(IdeSymbolsState state, LoadFileSymbolsSuccessAction action)
    {
        var views = new Dictionary<string, IdeSymbolsViewState>(state.Views, StringComparer.OrdinalIgnoreCase);
        var view = views.TryGetValue(action.SolutionId, out var existing)
            ? existing
    ...
    [ReducerMethod]
    public static IdeSymbolsState ReduceLoadSymbolsFailure(IdeSymbolsState state, LoadFileSymbolsFailureAction action)
    {
        var views = new Dictionary<string, IdeSymbolsViewState>(state.Views, StringComparer.OrdinalIgnoreCase);
        var view = views.TryGetValue(action.SolutionId, out var existing)
            ? existing
