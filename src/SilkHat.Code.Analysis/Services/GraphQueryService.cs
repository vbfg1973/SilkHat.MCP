...
namespace SilkHat.Code.Analysis.Services
{
    public sealed class GraphQueryService : IGraphQueryService
    {
        ...
        {
            if (_storeProvider.TryGet(solutionId, out var store) && store is not null) return store.ToSnapshot();
        ...
        {
            node = null;
            if (string.IsNullOrWhiteSpace(solutionId) || string.IsNullOrWhiteSpace(key)) return false;
        ...
        {
            if (!_storeProvider.TryGet(solutionId, out var store) || store is null) return Array.Empty<GraphNodeDto>();

            if (!store.TryGetNodeByKey(key, out var node) || node is null) return Array.Empty<GraphNodeDto>();
