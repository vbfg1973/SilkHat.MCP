...
public static class SymbolKeyUtility
{
    ...
    {
        if (ResolveSymbolKeyMethod is null)
        {
            return null;
        }

        var parameters = ResolveSymbolKeyMethod.GetParameters();
        object? resolution = parameters.Length switch
        {
            2 => ResolveSymbolKeyMethod.Invoke(null, new object?[] { symbolKey, compilation }),
            3 => ResolveSymbolKeyMethod.Invoke(null, new object?[] { symbolKey, compilation, CancellationToken.None }),
            _ => null
        };
    ...
    private static MethodInfo? ResolveResolveStringMethod()
    {
        ...
        return symbolKeyType
            ...
            .FirstOrDefault(method =>
            {
                if (!string.Equals(method.Name, "ResolveString", StringComparison.Ordinal))
                {
                    return false;
                }
                ...
                if (parameters[0].ParameterType != typeof(string) || parameters[1].ParameterType != typeof(Compilation))
                {
                    return false;
                }
