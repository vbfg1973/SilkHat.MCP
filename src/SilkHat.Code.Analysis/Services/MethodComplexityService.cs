...
namespace SilkHat.Code.Analysis.Services
{
    public sealed class MethodComplexityService : IMethodComplexityService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(docId)) return Task.FromResult<ComplexityResultDto?>(null);

            var method = DocumentationIdUtility.FindMethodByDocumentationId(solution, docId);
            if (method is null) return Task.FromResult<ComplexityResultDto?>(null);

            var syntaxRef = method.DeclaringSyntaxReferences.FirstOrDefault();
            if (syntaxRef is null) return Task.FromResult<ComplexityResultDto?>(null);
            ...
            if (compilation is null) return Task.FromResult<ComplexityResultDto?>(null);
