    ...
    public sealed class SolutionParser
    {
        public ParsedSolution Parse(string solutionPath)
        {
            ...
            foreach (var line in File.ReadLines(solutionPath))
            {
                ...
                if (!TryParseProjectLine(line, out var projectTypeId, out var name, out var relativePath,
                        out var projectId)) continue;
            ...
            out string projectId)
        {
            ...
            name = tokens[1];
            path = tokens[2];
            if (tokens.Count > 3) projectId = tokens[3];
        ...
        private static Guid? FindSolutionGuid(string solutionPath)
        {
            foreach (var line in File.ReadLines(solutionPath))
            {
                ...
                if (!trimmed.StartsWith("SolutionGuid", StringComparison.OrdinalIgnoreCase)) continue;
