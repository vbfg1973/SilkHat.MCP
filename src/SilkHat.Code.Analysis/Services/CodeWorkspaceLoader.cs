    ...
    public sealed class CodeWorkspaceLoader : ICodeWorkspaceLoader
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var resolvedSolutions = solutions
                ...
                .Select(group =>
                {
                    ...
                    if (group.Any(item => !string.Equals(item.Reference.SolutionId, first.Reference.SolutionId,
            ...
            foreach (var solutionEntry in resolvedSolutions)
            {
                ...
                if (solutionProjects.Count == 0)
                    throw new InvalidOperationException(
                        $"No C# projects were found in solution '{parsedSolution.SolutionPath}'.");
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    _statusStore?.SetJobRunning(solutionId, IndexJobType.ProjectsAndFiles,
                        "Building project compilations.");
                ...
                foreach (var project in workspaceProjects)
                {
                    ...
                    {
                        if (!projectKeyMap.TryGetValue(reference.ProjectId, out var referencedKey)) continue;
                ...
                foreach (var compilationEntry in compilations)
                {
                    ...
                    foreach (var symbol in EnumerateNamedTypes(compilation.GlobalNamespace))
                    {
                        ...
                        namedTypes.Add(dto);
                        if (!string.IsNullOrWhiteSpace(dto.Namespace)) namespaces.Add(dto.Namespace);

                        if (!namedTypeByKey.ContainsKey(dto.SymbolKey)) namedTypeByKey[dto.SymbolKey] = dto;
                        ...
                        if (locationAttributes is not null) typeLocationAttributes[typeKey] = locationAttributes;

                        memberNodes.AddRange(CollectMemberNodes(symbol, compilation, dto, projectIndex[projectKey].Name,
                            rootPath));
                    }
                }
                ...
                var relativeSolutionPath =
                    SolutionIdentity.NormalizeRelativePath(rootPath, parsedSolution.SolutionPath);
            ...
            IReadOnlyList<MemberNodeInfo> memberNodes)
        {
            ...
            foreach (var treeEntry in treeEntries)
            {
                ...
                if (treeEntry.Type == CodeTreeEntryType.File) repositoryPathToNodeId[treeEntry.RepositoryPath] = nodeId;
            }
            ...
            foreach (var (parent, children) in treeChildrenByParent)
            {
                ...
                {
                    if (!displayPathToNodeId.TryGetValue(child.DisplayPath, out var childNodeId)) continue;
            ...
            foreach (var type in namedTypes)
            {
                ...
                var typeAttributes = new Dictionary<string, string>
                {
                    ...
                    ["ProjectName"] = projects.TryGetValue(type.ProjectKey, out var projectIndex)
                        ? projectIndex.Name
                        : type.ProjectKey
                };
                if (!string.IsNullOrWhiteSpace(type.FilePath)) typeAttributes["RepositoryPath"] = type.FilePath;
                ...
                graph.AddNode(new GraphNodeDto(typeNodeId, GraphNodeKind.NamedType, typeKey, type.Name,
                    typeAttributes));
            ...
            {
                if (!typeNodeIds.TryGetValue(member.ParentKey, out var parentTypeId)) continue;
                ...
                if (!string.IsNullOrWhiteSpace(member.FilePath)) attributes["RepositoryPath"] = member.FilePath!;
                ...
                foreach (var parameter in member.Parameters)
                {
                    ...
                    graph.AddNode(new GraphNodeDto(parameterNodeId, GraphNodeKind.Parameter, parameter.Key,
                        parameter.Name, parameterAttributes));
        ...
        private static Workspace BuildWorkspace(IReadOnlyDictionary<string, ParsedProject> projects)
        {
            ...
            var projectIds = projects.ToDictionary(entry => entry.Key, _ => ProjectId.CreateNewId(),
                StringComparer.OrdinalIgnoreCase);
            ...
            {
                var projectId = projectIds[project.ProjectPath];
                foreach (var document in CreateDocuments(projectId, project)) solution = solution.AddDocument(document);

                foreach (var reference in project.ProjectReferences)
                {
                    if (!projectIds.TryGetValue(reference.FullPath, out var referencedId)) continue;
            ...
            CancellationToken cancellationToken)
        {
            ...
            await RunGitAsync(rootPath, solutionId, solutionPath, graphContext, cancellationToken)
                .ConfigureAwait(false);
            await RunComplexityAsync(rootPath, solutionId, solutionPath, workspace, graphContext, cancellationToken)
                .ConfigureAwait(false);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                _statusStore?.SetJobCompleted(solutionId, IndexJobType.Complexity,
                    "Complexity strategies not configured (skipped).");
                return;
            }
            ...
            try
            {
                ...
                var complexityData = await BuildComplexityDataAsync(rootPath, workspace, cancellationToken)
                    .ConfigureAwait(false);
            ...
            {
                _statusStore?.SetJobFailed(solutionId, IndexJobType.Complexity,
                    $"Complexity indexing failed: {ex.Message}");
            ...
            GraphContext context)
        {
            ...
            foreach (var reference in packageReferences)
            {
                ...
                if (!packageNodeIds.TryGetValue(packageKey, out var packageNodeId))
                {
                    ...
                    context.Graph.AddNode(new GraphNodeDto(packageNodeId, GraphNodeKind.Package, packageKey, packageKey,
                        packageAttributes));
            ...
            GraphContext context)
        {
            ...
            foreach (var commit in gitData.Commits)
            {
                ...
                if (commit.Date.HasValue) commitAttributes["Date"] = commit.Date.Value.ToString("O");

                context.Graph.AddNode(new GraphNodeDto(commitNodeId, GraphNodeKind.GitCommit, commit.Sha, commit.Sha,
                    commitAttributes));
                ...
                if (!authorNodeIds.TryGetValue(authorKey, out var authorNodeId))
                {
                    ...
                    context.Graph.AddNode(new GraphNodeDto(authorNodeId, GraphNodeKind.GitAuthor, authorKey,
                        commit.AuthorName, authorAttributes));
                ...
                foreach (var change in commit.Changes)
                {
                    ...
                    if (!context.RepositoryPathToNodeId.TryGetValue(normalizedPath, out var fileNodeId)) continue;
            ...
            GraphContext context)
        {
            foreach (var metric in complexityData.MethodMetrics)
            {
                ...
                context.Graph.AddNode(new GraphNodeDto(metricNodeId, GraphNodeKind.ComplexityMetric,
                    $"{metric.DocId}:{metric.Measure}", metric.Measure.ToString(), metricAttributes));
        ...
        private static IEnumerable<DocumentInfo> CreateDocuments(ProjectId projectId, ParsedProject project)
        {
            foreach (var filePath in project.CompileItems)
            {
                ...
                yield return DocumentInfo.Create(DocumentId.CreateNewId(projectId), name, filePath: filePath,
                    loader: loader);
            }
        }
        ...
        private static IReadOnlyList<MetadataReference> BuildDefaultReferences()
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies().Concat(seedAssemblies))
            {
                ...
                if (paths.Add(location)) references.Add(MetadataReference.CreateFromFile(location));
            }
            ...
            IReadOnlyDictionary<string, ProjectIndex> projectIndex)
        {
            ...
            {
                if (!projectPathToKey.TryGetValue(projectPath, out var projectKey)) continue;
        ...
        private static IEnumerable<INamedTypeSymbol> EnumerateNestedTypes(INamedTypeSymbol type)
        {
            ...
            {
                yield return nested;
                foreach (var deeper in EnumerateNestedTypes(nested)) yield return deeper;
            }
        }
        ...
        private static IReadOnlyDictionary<string, string>? BuildLocationAttributes(ISymbol symbol, string rootPath)
        {
            ...
            if (syntaxRef?.SyntaxTree?.FilePath is null)
            {
                ...
                if (location?.SourceTree?.FilePath is null) return null;

                var spanFallback = location.SourceSpan;
                var lineSpanFallback = location.GetLineSpan();
                var normalizedPathFallback =
                    SolutionIdentity.NormalizeRelativePath(rootPath, location.SourceTree.FilePath);
            ...
            string rootPath)
        {
            ...
            var parentKey = !string.IsNullOrWhiteSpace(typeDto.DocumentationId)
                ? typeDto.DocumentationId
                : typeDto.SymbolKey;
            ...
            IReadOnlyDictionary<ProjectId, string> projectKeyMap)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(project.FilePath)) continue;

                if (!projectKeyMap.TryGetValue(project.Id, out var projectKey)) continue;
                ...
                foreach (var document in project.Documents)
                {
                    ...
                    if (relativeToProject.StartsWith("..", StringComparison.Ordinal)) continue;
                ...
                foreach (var directory in directories.OrderBy(value => value, StringComparer.OrdinalIgnoreCase))
                {
                    var directoryFullPath =
                        Path.Combine(projectRoot, directory.Replace('/', Path.DirectorySeparatorChar));
        ...
        {
            if (entry.Type == CodeTreeEntryType.Project) return string.Empty;
        ...
        {
            if (_gitCommandRunner is null) return null;
            ...
            foreach (var line in lines)
            {
                if (line.StartsWith("COMMIT|", StringComparison.Ordinal))
                {
                    ...
                    currentDate = null;
                    if (DateTimeOffset.TryParse(parts.ElementAtOrDefault(4), CultureInfo.InvariantCulture,
                            DateTimeStyles.AssumeUniversal, out var parsedDate)) currentDate = parsedDate;
        ...
        private async Task<ComplexityGraphData?> BuildComplexityDataAsync(
            ...
            CancellationToken cancellationToken)
        {
            if (_complexityStrategyFactory is null) return null;
            ...
            var strategies = new Dictionary<ComplexityMeasureType, IComplexityStrategy>
            {
                [ComplexityMeasureType.Cognitive] =
                    _complexityStrategyFactory.GetStrategy(ComplexityMeasureType.Cognitive),
                [ComplexityMeasureType.Cyclomatic] =
                    _complexityStrategyFactory.GetStrategy(ComplexityMeasureType.Cyclomatic),
                [ComplexityMeasureType.Indentation] =
                    _complexityStrategyFactory.GetStrategy(ComplexityMeasureType.Indentation)
            };
