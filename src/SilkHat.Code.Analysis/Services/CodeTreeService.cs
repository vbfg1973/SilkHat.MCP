...
namespace SilkHat.Code.Analysis.Services
{
    public sealed class CodeTreeService : ICodeTreeService
    {
        ...
        private static GraphNodeDto? LookupNode(GraphStore store, string key)
        {
            ...
            if (store.TryGetNodeByKey(key, out var node) && node is not null) return node;
        ...
        {
            if (!attributes.TryGetValue("FilePath", out var path) || string.IsNullOrWhiteSpace(path)) return null;
