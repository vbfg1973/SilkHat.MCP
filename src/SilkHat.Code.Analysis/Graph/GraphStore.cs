...
namespace SilkHat.Code.Analysis.Graph
{
    ...
    {
        private readonly ConcurrentDictionary<(Guid SourceId, Guid TargetId, EdgeType Type), GraphEdgeDto> _edges =
            new();
        ...
        private readonly ConcurrentDictionary<GraphNodeKind, ConcurrentDictionary<string, Guid>> _nodeIdsByTypeAndKey =
            new();
        ...
        {
            if (node is null || node.IsEmpty) return false;

            if (_nodes.TryAdd(node.Id, node))
            {
                _graph.AddVertex(node.Id);
                _nodeIdsByKey[node.Key] = node.Id;
                var typed = _nodeIdsByTypeAndKey.GetOrAdd(node.Kind,
                    _ => new ConcurrentDictionary<string, Guid>(StringComparer.OrdinalIgnoreCase));
        ...
        public bool AddEdge(Guid sourceId, Guid targetId, EdgeType type,
            IReadOnlyDictionary<string, string>? attributes = null)
        {
            if (!_nodes.ContainsKey(sourceId) || !_nodes.ContainsKey(targetId)) return false;
            ...
            if (!_graph.AddEdge(edge)) return false;

            if (_edges.TryAdd(key, new GraphEdgeDto(sourceId, targetId, type, attributes))) return true;
