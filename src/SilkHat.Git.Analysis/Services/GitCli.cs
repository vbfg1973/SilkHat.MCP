...
namespace SilkHat.Git.Analysis.Services
{
    public sealed class GitCli : IGitCli
    {
        ...
        {
            var listResult = await _runner.ExecuteAsync(repoRoot, new[] { "ls-tree", "-r", "--name-only", "HEAD" },
                cancellationToken);
            ...
            foreach (var file in files)
            {
                if (typeFilter.HasValue && typeFilter != GitTreeEntryType.File) continue;

                if (!MatchesNameFilter(file, nameFilter)) continue;

                var lastChange = await GetLastChangeAsync(cache, repoRoot, file, cancellationToken);
                if (!MatchesChangeFilters(lastChange, changedAfter, author)) continue;
            ...
            if (includeDirectories)
                ...
                {
                    if (typeFilter.HasValue && typeFilter != GitTreeEntryType.Directory) continue;

                    if (!MatchesNameFilter(directory, nameFilter)) continue;
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                if (!cache.CommitFiles.TryGetValue(entry.CommitSha, out var files)) continue;

                foreach (var file in files)
                {
                    if (string.Equals(file, targetKey, StringComparison.OrdinalIgnoreCase)) continue;
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(author)) args.Add($"--author={author}");

            if (since.HasValue) args.Add($"--since={since.Value:O}");

            if (until.HasValue) args.Add($"--until={until.Value:O}");

            if (merge.HasValue) args.Add(merge.Value ? "--merges" : "--no-merges");
            ...
            CancellationToken cancellationToken)
        {
            var result = await _runner.ExecuteAsync(
                ...
                new[]
                {
                    "log", "--date=iso-strict", "--pretty=format:COMMIT|%H|%ad|%an|%s", "--name-status", "--",
                    normalizedKey
                },
                cancellationToken);
            EnsureSuccess(result, "git log");
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(author)) countArgs.Add($"--author={author}");

            if (since.HasValue) countArgs.Add($"--since={since.Value:O}");

            if (until.HasValue) countArgs.Add($"--until={until.Value:O}");

            if (merge.HasValue) countArgs.Add(merge.Value ? "--merges" : "--no-merges");
            ...
            if (int.TryParse(countResult.StandardOutput.Trim(), out var total)) return total;
        ...
        private static IReadOnlyList<GitFileDiffLineDto> ParseDiffLines(string output)
        {
            ...
            foreach (var rawLine in output.Split(new[] { '\r', '\n' }, StringSplitOptions.None))
            {
                ...
                {
                    if (TryParseHunkStart(rawLine, out var newStart)) currentLine = newStart;
                    continue;
                }
        ...
        private async Task<GitPathChangeDto?> GetLastChangeAsync(
            ...
            CancellationToken cancellationToken)
        {
            if (cache.PathMetadata.TryGetValue(path, out var cached)) return cached;
            ...
            GitRepositoryCache cache)
        {
            ...
            foreach (var rawLine in output.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                {
                    if (current is not null) entries.Add(current.Build(targetPath, cache));
            ...
            foreach (var entry in entries)
            {
                ...
                foreach (var change in entry.Changes)
                {
                    ...
                    var (additions, deletions) =
                        await GetNumStatAsync(repoRoot, entry.CommitSha, pathKey, cancellationToken);
        ...
        {
            if (value == "-" || string.IsNullOrWhiteSpace(value)) return null;
        ...
        private sealed class GitHistoryBuilder
        {
            ...
            public static GitHistoryBuilder FromHeader(string header)
            {
                ...
                if (parts.Length < 5) throw new InvalidOperationException("Unexpected git log header format.");
            ...
            public void AddChange(string line, string targetPath, GitRepositoryCache cache)
            {
                ...
                foreach (var rawPath in paths)
                {
                    ...
                    if (IsTargetPath(normalized, targetPath))
                        _changes.Add(new GitFileChangeDto(NormalizePath(normalized), status, changeKind, null, null, 0,
                            0));
                }
            ...
            {
                if (string.IsNullOrWhiteSpace(status)) return GitChangeKind.Unknown;
    ...
    public static class GitCommitParser
    {
        public static IReadOnlyList<GitCommitDto> Parse(string output)
        {
            ...
            foreach (var rawLine in output.Split(new[] { '\r', '\n' }, StringSplitOptions.None))
            {
                if (rawLine.StartsWith("COMMIT|", StringComparison.Ordinal))
                {
                    if (current is not null) commits.Add(current.Build());
        ...
        private sealed class GitCommitBuilder
        {
            ...
            public static GitCommitBuilder FromHeader(string header)
            {
                ...
                if (parts.Length < 8) throw new InvalidOperationException("Unexpected git log header format.");
