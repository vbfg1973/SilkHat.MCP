                ...
                Task<SymbolDescriptionResult<T>>> handler)
        {
            if (string.IsNullOrWhiteSpace(docId))
                return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed",
                    "DocumentationId is required.", "Validation");
            ...
            return result.Status switch
            {
                ...
                SymbolDescriptionStatus.Unsupported => ProblemWithCategory(StatusCodes.Status400BadRequest,
                    "Unsupported", result.Error ?? "DocumentationId is not supported.", "Code"),
                _ => ProblemWithCategory(StatusCodes.Status404NotFound, "Not Found",
                    result.Error ?? "Symbol not found.", "Code")
            };
        }
    }
}