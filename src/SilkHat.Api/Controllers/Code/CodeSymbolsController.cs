...
namespace SilkHat.Api.Controllers
{
    ...
    public sealed class CodeSymbolsController : ApiControllerBase
    {
        ...
        [HttpPost("lookup")]
        public ActionResult<SymbolLookupResultDto> LookupSymbol(Guid id, string solutionId,
            [FromBody] SymbolLookupRequest request)
        {
            ...
            if (graphMatch is not null) return Ok(graphMatch);
        ...
        private SymbolLookupResultDto? LookupInGraph(string solutionId, SymbolLookupRequest request)
        {
            ...
            if (node is null || node.Kind != GraphNodeKind.NamedType) return null;
        ...
        {
            if (string.IsNullOrWhiteSpace(expectedKind)) return true;
