...
namespace SilkHat.Api.Controllers
{
    ...
    public sealed class RepositoryGroupsController : ApiControllerBase
    {
        ...
        public async Task<ActionResult<RepositoryGroupDto>> GetById(Guid id, CancellationToken cancellationToken)
        {
            ...
            if (group is null)
                return ProblemWithCategory(StatusCodes.Status404NotFound, "Not Found", "Repository group not found.",
                    "NotFound");
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var config in group.RepositoryConfigs.OrderBy(config => config.Name))
            {
                ...
                await foreach (var evt in _processor.ExecuteAsync(command, context, cancellationToken)) lastEvent = evt;
                ...
                if (loaded) _ = _metricsPrecomputeService.StartPrecomputeAsync(config.Id, CancellationToken.None);
            }
