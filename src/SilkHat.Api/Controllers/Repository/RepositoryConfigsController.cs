    ...
    public sealed class RepositoryConfigsController : ApiControllerBase
    {
            ...
            CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(request.Name))
                return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed", "Name is required.",
                    "Validation");

            if (string.IsNullOrWhiteSpace(request.RootPath))
                return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed",
                    "RootPath is required.", "Validation");

            if (!TryValidateSolutions(request.Solutions, out var solutionError))
                return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed",
                    solutionError ?? "Solutions are required.", "Validation");

            if (!RepositoryInputNormalization.TryNormalizeRootPath(request.RootPath, out var normalizedPath,
                    out var error))
                return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed",
                    error ?? "RootPath is invalid.", "Validation");

            if (!_discovery.TryValidateRepositoryPath(normalizedPath, out var validationError))
                return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed",
                    validationError ?? "RootPath is invalid.", "Validation");
            ...
            if (request.GroupId.HasValue)
            {
                ...
                if (!groupExists)
                    return ProblemWithCategory(StatusCodes.Status400BadRequest, "Validation Failed",
                        "GroupId does not match an existing group.", "Validation");
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var solution in updatedSolutions) config.Solutions.Add(solution);
        ...
        private List<RepositorySolutionConfig> BuildSolutions(RepositoryConfig config,
            IReadOnlyList<RepositorySolutionDto>? solutions)
        {
            var results = new List<RepositorySolutionConfig>();
            if (solutions is null) return results;

            foreach (var solution in solutions
                         .GroupBy(solution => NormalizeSolutionPath(solution.RelativePath),
