...
public sealed class DecisionsController : ApiControllerBase
{
    ...
    {
        if (!TryGetWorkspace(id, solutionId, out var workspace, out var solution, out var error))
        {
            return error!;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (payload is null)
        {
            return ProblemWithCategory(StatusCodes.Status400BadRequest, "Invalid Request", "Payload is invalid for decision type.", "Validation");
        }
        ...
        if (decision is null)
        {
            return ProblemWithCategory(StatusCodes.Status404NotFound, "Not Found", "Decision not found.", "Decisions");
        }
