...
namespace SilkHat.Api.Services
{
    ...
    public sealed class CodeTreeQueryService : ICodeTreeQueryService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var children = (await _treeService.GetTreeAsync(workspace, solution, query.ParentId, cancellationToken))
                .ToList();
            ...
            int threshold)
        {
            ...
            if (fileValues.Count == 0 || snapshot.Nodes.Count == 0) return included;
            ...
            IReadOnlyDictionary<Guid, GraphNodeDto> nodesById)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                if (!string.IsNullOrWhiteSpace(key)) included.Add(key);
