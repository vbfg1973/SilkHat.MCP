...
public sealed class DecisionService : IDecisionService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (type.HasValue)
        {
            query = query.Where(decision => decision.DecisionType == type.Value);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (active.HasValue)
        {
            query = query.Where(decision => decision.IsActive == active.Value);
        }
        ...
        CancellationToken cancellationToken)
    {
        foreach (var method in EnumerateInterfaceMethods(solution))
        {
            ...
            var interfaceTypeDocId = DocumentationIdUtility.GetDocumentationId(method.ContainingType) ?? string.Empty;
            ...
            var payload = new ResolveInterfaceDecisionPayloadDto(
                ...
                BuildCandidates(solution, resolution.CandidateTypeNames, resolution.CandidateMethodDocumentationIds),
                null,
                null);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var legacyDecision in legacy)
        {
            ...
            if (existingKeySet.Contains(subjectKey))
            {
                continue;
            }
    ...
    private static bool ValidatePayload(DecisionType type, string payloadJson, CodeSolutionWorkspace solution)
    {
        ...
        if (!IsDocIdValid(payload.InterfaceTypeDocId, solution, DocumentationIdUtility.FindTypeByDocumentationId))
        {
            return false;
        }

        if (!IsDocIdValid(payload.InterfaceMethodDocId, solution, DocumentationIdUtility.FindMethodByDocumentationId))
        {
            return false;
        }
        ...
        foreach (var candidate in payload.Candidates)
        {
            ...
            if (!string.IsNullOrWhiteSpace(candidate.MethodDocId)
                && !IsDocIdValid(candidate.MethodDocId, solution, DocumentationIdUtility.FindMethodByDocumentationId))
            {
                return false;
            }
        }
        ...
        if (!string.IsNullOrWhiteSpace(payload.SelectedMethodDocId)
            && !IsDocIdValid(payload.SelectedMethodDocId, solution, DocumentationIdUtility.FindMethodByDocumentationId))
        {
            return false;
        }
