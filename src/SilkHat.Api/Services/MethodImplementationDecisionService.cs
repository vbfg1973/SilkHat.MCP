    ...
    public sealed class MethodImplementationDecisionService : IMethodImplementationDecisionService
    {
            ...
            CancellationToken cancellationToken)
        {
            if (interfaceMethod.ContainingType.TypeKind != TypeKind.Interface
                ...
                return new MethodImplementationResolution(null, null, false, Array.Empty<string>(),
                    Array.Empty<string?>());
            ...
            if (!ignoreStoredDecisions)
            {
                ...
                {
                    var resolved =
                        ResolveImplementationByDocId(solution, storedDecision.ImplementationMethodDocumentationId)
            ...
            return new MethodImplementationResolution(
                ...
                null,
                true,
                nonTestCandidates.Select(candidate => candidate.TypeName).Distinct(StringComparer.OrdinalIgnoreCase)
                    .ToList(),
                nonTestCandidates.Select(candidate => candidate.MethodDocumentationId)
        ...
        {
            if (string.IsNullOrWhiteSpace(implementationMethodDocId)) return null;
            ...
            string typeName)
        {
            foreach (var compilation in solution.Compilations.Values)
            {
                ...
                var interfaceMember =
                    interfaceType is null ? null : ResolveInterfaceMember(interfaceType, interfaceMethod);

                foreach (var candidate in EnumerateNamedTypes(compilation.GlobalNamespace))
                {
                    var candidateName = GetTypeDisplayName(candidate);
                    if (!string.Equals(candidateName, typeName, StringComparison.Ordinal)) continue;
            ...
            string payloadJson)
        {
            ...
            {
                var resolved =
                    DocumentationIdUtility.FindMethodByDocumentationId(solution, payload.SelectedMethodDocId);
            ...
            string? interfaceTypeDocId)
        {
            ...
            foreach (var (compilation, root) in EnumerateCandidateRoots(solution, interfaceMethod))
            {
                ...
                {
                    if (candidate.TypeKind == TypeKind.Interface || candidate.IsAbstract) continue;

                    if (!ImplementsInterface(candidate, interfaceType, interfaceTypeName, interfaceTypeDocId)) continue;
            ...
            string interfaceNamespace)
        {
            ...
            foreach (var candidate in EnumerateNamedTypes(root))
            {
                ...
                if (!string.IsNullOrWhiteSpace(interfaceNamespace)
                    && !string.Equals(candidate.ContainingNamespace?.ToDisplayString(), interfaceNamespace,
        ...
        {
            if (interfaceMethod.AssociatedSymbol is not null) return interfaceMethod.AssociatedSymbol;
        ...
        {
            if (implementation is IMethodSymbol method) return method;
            ...
            ISymbol? interfaceMember)
        {
            ...
            if (explicitDocIdMatch is not null) return explicitDocIdMatch;
            ...
            if (interfaceMethod.MethodKind is MethodKind.PropertyGet or MethodKind.PropertySet)
            {
                ...
                if (!string.IsNullOrWhiteSpace(propertyName))
                {
                    var candidateProperty = candidate.GetMembers(propertyName)
                        .OfType<IPropertySymbol>()
                        .FirstOrDefault();
                    if (candidateProperty is null) return null;
            ...
            if (interfaceMethod.MethodKind is MethodKind.EventAdd or MethodKind.EventRemove)
            {
                ...
                if (!string.IsNullOrWhiteSpace(eventName))
                {
                    var candidateEvent = candidate.GetMembers(eventName)
                        .OfType<IEventSymbol>()
                        .FirstOrDefault();
                    if (candidateEvent is null) return null;
