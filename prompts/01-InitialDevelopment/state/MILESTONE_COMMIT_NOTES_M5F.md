...
- implement hierarchical code tree queries with parentId filtering and lazy-loading in the IDE
- add FluentValidation for tree query validation and tests for hierarchy/validation behavior
- sort solution list alphabetically before selecting default solution in the IDE
- align FluentValidation.AspNetCore package version with repository constraints (11.3.1)
- fix MudIconButton OnClick binding to use typed mouse event callbacks in the IDE tree
- precompute solution tree parent/child mappings to speed up lazy tree queries
- replace the IDE tree list with MudTreeView and ensure node names render correctly
- fix MudTreeView OnClick binding and tree parent path calculation build errors
- order tree children with directories before files and add test coverage for the ordering
- remove UI-side tree sorting so server ordering is preserved; fix test helper path parsing
...
- src/SilkHat.Code.Analysis/Services/CodeTreeService.cs
- src/SilkHat.Code.Analysis/Abstractions/ICodeTreeService.cs
...
- src/SilkHat.Api/Models/CodeTreeQuery.cs
- src/SilkHat.Api/Validation/CodeTreeQueryValidator.cs
...
- tests/SilkHat.Api.Tests/Validation/CodeTreeQueryValidatorTests.cs
...
- tests/SilkHat.Tests/Services/CodeTreeServiceTests.cs
...
- code tree queries now return only top-level projects by default and use parentId to fetch direct descendants
- IDE default solution selection is alphabetical by name (ties by relative path)
